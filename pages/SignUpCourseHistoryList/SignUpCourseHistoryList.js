...
//请求数据
var loadSignUpCourseHistoryMore = function (that) {
  that.data.hasSignUpCourseHistoryMore = false;
  if (that.data.isSignUpCourseHistoryDown) {
    console.log("daodi-----");
    return;
  }
  that.setData({
  ...
  wx.request({
    url: app.globalData.urlPath + 'course/getStudentSignUpCourseHistoryList',
    data: {
      page: page,
      limit: page_size,
    ...
    success: function (res) {
      ...
      if (res.data.data == null || res.data.data == [] || res.data.data.length == 0) {
        ...
        that.setData({
          hidden: true,
          isSignUpCourseHistoryDown: true,
        });
        return;
      }
      ...
      that.setData({
        list: list,
        hasSignUpCourseHistoryMore: true,
        hidden: true,
      });
...
Page({
  ...
  data: {
    ...
    hidden: true,
    list: [],
    isSignUpCourseHistoryDown: false,
    hasSignUpCourseHistoryMore: false,
    studentId: "",
  },
  ...
  onLoad: function (options) {
    ...
    //数据刷新
    page = 1;
    loadSignUpCourseHistoryMore(that);
  },
  //下拉加载
